    ...
    <div className="space-y-5">
            ...
            {variants.map((v) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex w-full items-center justify-between rounded-xl border p-4 text-left transition-all disabled:cursor-not-allowed disabled:opacity-50",
                    isActive
                      ? "border-forest bg-paper shadow-[0_4px_14px_-8px_rgba(10,22,40,0.25)]"
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={cn(
                        ...
                        isActive ? "border-forest bg-forest" : "border-ink/25 bg-paper",
                      )}
                    >
                      {isActive ? <span className="h-2 w-2 rounded-full bg-cream" /> : null}
                    </span>
                    <div>
                      <div className="font-display text-[17px] text-ink">{v.label}</div>
                  ...
                  <Money pence={v.price_pence} className="font-display text-[22px] text-forest" />
                </button>
              );
            })}
          </div>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {added ? null : <span className="ml-0.5 opacity-80">£{(lineTotal / 100).toFixed(2)}</span>}
        </button>
      </div>
    </div>
  );
