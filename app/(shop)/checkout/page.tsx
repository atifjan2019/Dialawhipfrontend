...
import type { Address, PricingResult } from "@/lib/types";
...
export default function CheckoutPage() {
  ...
  const [addr, setAddr] = useState({ label: "Home", line1: "", line2: "", city: "Newcastle upon Tyne", postcode: "" });
  ...
  }, [items, addresses, selectedAddress]);
    ...
    <div className="mx-auto max-w-6xl px-6 py-14">
      ...
      <form onSubmit={onSubmit} className="mt-12 grid gap-10 lg:grid-cols-[1fr_380px]">
        <div className="space-y-10">
          ...
          <Section step="02" title="When would you like it?">
            <div className="rounded-lg border hairline bg-paper p-6">
              <Field label="Scheduled for (optional)">
                <Input type="datetime-local" value={scheduled} onChange={(e) => setScheduled(e.target.value)} />
                <FieldError>{errors.scheduled_for?.[0]}</FieldError>
              </Field>
              <p className="mt-3 text-[12px] text-ink-muted">Leave blank for "as soon as possible." Orders require 24 hours' notice.</p>
            </div>
          </Section>

          <Section step="03" title="Anything we should know?">
            <div className="rounded-lg border hairline bg-paper p-6">
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Allergies, gate codes, special requests…" />
            </div>
          </Section>
        ...
        <aside className="h-fit space-y-5 lg:sticky lg:top-24">
          <div className="rounded-lg border hairline bg-paper p-6">
            ...
            <div className="mt-6 space-y-2 border-t hairline pt-5 text-[13px]">
              ...
              <Row label="Delivery"><Money pence={pricing?.delivery_fee_pence ?? 0} /></Row>
            ...
            <p className="mt-3 text-center text-[11px] text-ink-muted">Powered by Stripe · Secured with TLS</p>
          </div>
        </aside>
      </form>
    </div>
