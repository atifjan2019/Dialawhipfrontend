    ...
    <div className="mx-auto max-w-[1280px] px-6 py-14">
      ...
      <div className="mt-6 flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-3 max-w-lg text-[14px] leading-relaxed text-ink-muted">
            All {prods.data.length} {category.name.toLowerCase()} we stock, sorted by
            popularity. 20-minute delivery across Newcastle.
          </p>
        </div>

        <Link
          href="/shop"
          className="inline-flex h-10 items-center rounded-full border hairline bg-paper px-4 text-[12px] font-medium text-ink-soft transition-colors hover:border-brand hover:text-brand"
        >
          ← All categories
        </Link>
      </div>
      ...
      {prods.data.length === 0 ? (
        <div className="mt-16 rounded-2xl border hairline bg-paper p-12 text-center">
          <p className="text-[20px] font-bold text-ink">Nothing in stock right now.</p>
          <p className="mt-2 text-[14px] text-ink-muted">Check back soon — we restock weekly.</p>
        </div>
      ) : (
